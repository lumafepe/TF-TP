...
class Raft:
    ...
    def fromClient(self, msg: Message):
        ...
        if self.node_id == self.leaderId:
            ...
            if (
                ...
                and self.log[self.nextCommit].term == self.currentTerm
            ):
    ...
    def heartbeat_thread(self) -> None:
        if self.heartbeat_running and time() - self.lastHeartbeat >= 0.1:
            ...
            self.gossip_kwargs(
                ...
                prevLogTerm=self.log[self.commitIndex].term,
                entries=self.log[self.commitIndex + 1 :],
                leaderCommit=self.commitIndex,
                leaderRound=self.roundLC,
                isRPC=False,
                bitmap=self.bitmap,
                nextCommit=self.nextCommit,
                maxCommit=self.maxCommit,
            )
            # else:
    ...
    def change_role(self, role: RaftRole):
        ...
        match role:
            case RaftRole.LEADER:
                ...
                self.nextIndex = {
                    k: len(self.log) for k in self.node_ids if k != self.node_id
                }
    ...
    def request_vote(self, msg: Message):
        ...
        voteGranted = (
            self.votedFor is None or self.votedFor == msg.body.candidateId
        ) and self.more_up_to_date(msg.body.lastLogIndex, msg.body.lastLogTerm)
    ...
    def update(self) -> None:
        if sum(self.bitmap) >= (self.node_count + 2) // 2:
            ...
            if (
                self.nextCommit >= len(self.log) - 1
                or self.log[-1].term != self.currentTerm
            ):
                self.nextCommit += 1
            else:
                self.nextCommit = len(self.log) - 1
    ...
    def gossip(self, msg: Message):
        for i in range(self.fanout):
            ...
            send(
                self.node_id,
                self.node_permutation[(self.c + i) % (self.node_count - 1)],
                **body,
            )
        self.c += self.fanout
    ...
    def append_entries(self, msg: Message) -> None:
        ...
        if (
            len(self.log) <= msg.body.prevLogIndex
            or self.log[msg.body.prevLogIndex].term != msg.body.prevLogTerm
        ):
            send(
                self.node_id,
                self.leaderId,
        ...
        if not msg.body.isRPC:
            ...
            send(
                ...
                term=self.currentTerm,
                success=True,
                lastLogIndex=min(len(self.log) - 1, msg.body.prevLogIndex + 1),
            )
    ...
    def append_entries_reply(self, msg: Message) -> None:
        ...
        if msg.body.success and self.matchIndex[msg.src] < msg.body.lastLogIndex:
            ...
            reply(
                msg,
                type="AppendEntriesRPC",
                ni=self.nextIndex[msg.src],
                term=self.currentTerm,
                leaderId=self.node_id,
                prevLogIndex=max(self.nextIndex[msg.src] - 1, 0),
    ...
    def conflicts(self, entry: Log, index: int) -> bool:
        ...
        return index < len(self.log) and (
            self.log[index].term != entry.term or entry in self.log
        )
